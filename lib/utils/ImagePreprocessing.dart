  ...
  static Future<img.Image> applySegmentation(img.Image image) async {
    ...
    try {
      ...
      if (_outputType == TensorType.int32 || _outputType == TensorType.int64) {
        ...
        } else if (outputTensor is List<List<List<int>>>) {
        ...
        for (int i = 0; i < segWidth * segHeight; i++) {
          if (i >= outputIntDataFlat.length) break;
          int y = i ~/ segWidth;
          int x = i % segWidth;
      ...
      for (int y = 0; y < image.height; y++) {
        for (int x = 0; x < image.width; x++) {
          var maskPixelValue = finalMask.getPixel(x, y);
          int alpha = maskPixelValue.a.toInt();
          if (alpha > 128) {
